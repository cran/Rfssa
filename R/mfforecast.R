...
mfforecast <- function(U, groups = list(c(1)), h = 1, method = "recurrent", tol = 10^-3) {
  ...
  for (a in 1:length(groups)) {
    ...
    for (j in 1:p) {
      ...
      shifter[2, j+1] <- shifter[2, j] + ncol(U$Y@B[[j]])
    ...
    if (method == "recurrent") {
      ...
      for (m in 1:h) {
        for (j in 1:(L - 1)) {
          ...
          for (q in 1:p) {
            ...
            my_obs[(shifter[1, (q+1)]:shifter[2, (q+1)]), 1] <- Q[[1]]@C[[q]][, (N + j - L + m)]
          }
          A_j <- Neu %*% D %*% t(E_j)
          fssa_fore[, m] <- fssa_fore[, m] + A_j %*% my_obs
        }
        for(q in 1:p) Q[[1]]@C[[q]] <- cbind(Q[[1]]@C[[q]], fssa_fore[(shifter[1, (q+1)]:shifter[2, (q+1)]), m]);
      }
      for (q in 1:p) {
        out_g[[q]] <- basis[[q]] %*% fssa_fore[(shifter[1, (q+1)]:shifter[2, (q+1)]), ]
      }
      fts_out <- Rfssa::fts(out_g, basis, U$Y@grid)
      ...
      for (j in 1:p) {
        ...
        Lshifter[2, (j+1)] <- Lshifter[2, j] + (L-1)*ncol(U$Y@B[[j]])
        for (n in 1:k) {
